...
프로젝트를 다운로드하고 로컬 환경에서 실행하려면 아래의 명령어를 입력하세요.
...
서버가 실행되면 브라우저에서 `http://localhost:5173` 으로 접속하여 앱을 확인할 수 있습니다.
